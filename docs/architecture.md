...
- Redmine API 通信は通常 native-tls を使い、`ssl_no_revoke = true` または `--ssl-no-revoke` 指定時だけ rustls に切り替えます。
...
- HTTPS 接続に失敗しても HTTP へ自動 downgrade しません。API key の平文送信を避けるため、no-ssl fallback を追加する場合も明示的な unsafe option として扱います。
